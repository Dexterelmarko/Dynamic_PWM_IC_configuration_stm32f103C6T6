...
// UART
uint8_t dma_uart1_rx_buf[BUF_SIZE];
...
int main(void)
{
  ...
  HAL_UART_Receive_DMA(&huart1, dma_uart1_rx_buf, BUF_SIZE);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, SPI1_RESET_Pin|SPI1_NSS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : SPI1_RESET_Pin SPI1_NSS_Pin */
  GPIO_InitStruct.Pin = SPI1_RESET_Pin|SPI1_NSS_Pin;
...
void IC_config(TIM_HandleTypeDef *htim, uint32_t TIM_CHANNEL, GPIO_TypeDef *GPIO_Port, uint32_t Pin){
	...
	__HAL_TIM_SET_COUNTER(htim, 0);
	__HAL_TIM_SET_PRESCALER(htim, 71);
